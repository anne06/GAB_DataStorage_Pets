...
public class EditorActivity
    ...
    private void deletePet() {
        if (!isEdit) {
            // We are in INSERT mode, the PET does not already exists
            return;
        }

        int nbPetDeleted = getContentResolver().delete(mCurrentPetUri, null, null);
        if (nbPetDeleted == 1) {
            Toast.makeText(this,
    ...
    /********************************************************
     *
     * Dialog if we live without saving data
     *
     *********************************************************/
